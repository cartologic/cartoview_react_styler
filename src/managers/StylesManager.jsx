...
class StylesManager {
  ...
  getStyle(layerName, styleName, title){
    if(styleName == "new"){
      return this._getNewStyle(layerName, title).then(styleName => this._styles[styleName]);
    }
    ...
    return WPSClient.vecFeatureClassStats(layerName, {
      ...
      classes.forEach((c, index) => {
        const rule = new OpenLayers.Rule({
          symbolizers: [this._getSymbolizer(config, index, size, 7)],
          title: c.label,
      ...
      styleObj.namedLayers[styleObj.name].userStyles[0].rules = rules;
      return styleObj;
    });
  }
  createUniqueRules(styleObj, config){
    ...
    return WPSClient.gsUnique(layerName, attribute).then((res) => {
      ...
      res.features.slice(0, MAX_SIZE).forEach((f, index) => {
        ...
        const rule = new OpenLayers.Rule({
          symbolizers: [this._getSymbolizer(config, index, size, MAX_SIZE)],
          title: value,
      ...
      if(size > MAX_SIZE){
        const rule = new OpenLayers.Rule({
          symbolizers: [this._getSymbolizer(config, MAX_SIZE, size, MAX_SIZE)],
          title: "Others"
        });
        rules.unshift(rule);
      }
  ...
  _getSymbolizer(config, index, size, defaultSize){
    ...
    const s = new OpenLayers.Symbolizer[layerType]({
      strokeColor: this._getColor('stroke', config, index, size, defaultSize),
      strokeWidth: config.strokeWidth,
      strokeOpacity: config.strokeOpacity
    });
    if(layerType != "Line"){
      s.fill = true;
      s.fillColor = this._getColor('fill', config, index, size, defaultSize),
